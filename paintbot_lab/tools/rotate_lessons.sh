...
BUFFER="$REPO/paintbot_lab/TENTATIVE_LESSONS.md"
ARCHIVE_DIR="$REPO/paintbot_lab/lessons_archive"
...
if [[ "$SOURCE" != "startup" && "$SOURCE" != "clear" ]]; then
  ...
  emit_context "Tentative-lessons buffer (this session's, write lessons AS YOU GO): paintbot_lab/TENTATIVE_LESSONS.md"
  exit 0
fi
...
if [[ -n "$ARCHIVED" ]]; then
  CTX="Tentative-lessons buffer rotated: previous session's lessons archived to paintbot_lab/lessons_archive/$ARCHIVED. Fresh buffer: paintbot_lab/TENTATIVE_LESSONS.md — write candidate lessons there AS YOU GO."
elif [[ -n "$SKIPPED_DUP" ]]; then
  CTX="Fresh tentative-lessons buffer: paintbot_lab/TENTATIVE_LESSONS.md (previous buffer was a byte-identical copy of an existing archive - likely restored by a git sync; not re-archived) - write candidate lessons there AS YOU GO."
else
  CTX="Fresh tentative-lessons buffer: paintbot_lab/TENTATIVE_LESSONS.md (previous buffer was empty; nothing archived) — write candidate lessons there AS YOU GO."
fi

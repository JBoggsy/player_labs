...
record_state() {  # let the Stop nudge detect "buffer untouched this session"
  { md5 -q "$BUFFER" 2>/dev/null || md5sum "$BUFFER" 2>/dev/null | cut -d' ' -f1; } \
    > "$STATE_DIR/claude_cnw_lessons_baseline_$SESSION_ID" 2>/dev/null || true
}
...
if [[ "$SOURCE" != "startup" && "$SOURCE" != "clear" ]]; then
  ...
  [[ -f "$STATE_DIR/claude_cnw_lessons_baseline_$SESSION_ID" ]] || record_state
...
if [[ -n "$ARCHIVED" ]]; then
  CTX="Tentative-lessons buffer rotated: previous session's lessons archived to cue_n_woo_lab/lessons_archive/$ARCHIVED. Fresh buffer: cue_n_woo_lab/TENTATIVE_LESSONS.md — write candidate lessons there AS YOU GO (a Stop hook will nudge once if substantive work happens with the buffer untouched)."
else

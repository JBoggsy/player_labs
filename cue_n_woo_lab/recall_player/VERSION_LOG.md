...
| v4 | **League-DQ root-cause fix:** the in-flight `pending` guard outlived its phase. After the 3rd probe we held `pending="ask"`; `_settle_pending` only cleared on `me.judge>=3`, but the proposals state arrives with me.judge LAGGING → guard never cleared → propose blocked → global phase stalls → inactive -100. Fix: track `_pending_phase`, drop the guard the instant the server's phase advances past the action's phase. Carries v3 phrase-recall. | **Wedge FIXED:** live MIRROR race (the DQ-repro config) completes cleanly, zero timeouts (4/5 league eps complete; the 1 timeout was a DIFFERENT mode — judge too slow to answer our 3rd probe in `private_questions` under load, hits everyone). Submitted to league (sub_c8b86fb6). Open: phrase-recall ties/loses to gabby head-to-head. |
| v5 | **Beat-gabby attempt:** (1) register-forcing probe — make the judge emit gabby's winning shape, a 3-5 word surreal declarative "The <noun> <verb> <object>" (scored 1.00 live; our wordier phrases scored ~0.56). (2) commit ONE best phrase everywhere (heuristic `phrase_score`) instead of cycling all three. Shorter phrases also generate faster → lower timeout risk. 11 tests pass. | **REGRESSION — DO NOT PROMOTE.** 0/6 vs gabby; our-question score collapsed 197→46/ep. Two causes: (a) the rigid template makes the judge emit GENERIC phrases ("The gavel ink-stamps the gossip") that COLLIDE with gabby's and lose the crispness tiebreak 0.00; (b) one-phrase reuse REMOVED v4's diversity hedge, so a single weak/colliding phrase tanks all 3 rows (v4 cycled 3, so a bad one cost 1 row). v4 stays champion. |

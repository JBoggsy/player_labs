...
| v5 | **Beat-gabby attempt:** (1) register-forcing probe — make the judge emit gabby's winning shape, a 3-5 word surreal declarative "The <noun> <verb> <object>" (scored 1.00 live; our wordier phrases scored ~0.56). (2) commit ONE best phrase everywhere (heuristic `phrase_score`: starts-with-The + 3-5 words + no trailing punct) instead of cycling all three (avoids planting our weak ones). Shorter phrases also generate faster → lower timeout risk. 11 tests pass. | Racing vs gabby (xreq_bbbe5e57, 6 eps). |

...
Artifact types include `results`, `replay`, `logs`, `error-info`, `spec`, `player-status`, `game-config`. Its pool-discovery mode is unsupported: choose a round, division or experience request. A `replay_url` is a watch link and must not be downloaded as replay bytes.

...
## build — `crewrift_lab/tools/build_player.sh crewborg` (→ `players-crewborg:dev`)

- **linux/amd64 mandatory** — the runner (`runner.py:_assert_linux_amd64_image`) hard-fails on arm64.
- The Dockerfile copies the `crewborg` package and sets `CMD ["python","-m","crewrift.crewborg.coworld.policy_player"]`;
  the SDK is installed from the public `Metta-AI/coworld-tools` repo (see `crewrift/crewborg/coworld/Dockerfile`).
...
- **`--run`** (repeatable, one token per flag, e.g. `--run python --run -m --run crewrift.crewborg.coworld.policy_player`)
...
- `--use-bedrock` (+ optional `--bedrock-model`) — backend access. In a hosted pod crewborg gates on
  ...
  section of [`coworld-platform.md`](../../../../crewrift_lab/docs/coworld-platform.md#bedrock--in-pod-llm).
- `--secret-env CREWBORG_LLM_MEETINGS=1` and/or `--secret-env CREWBORG_LLM_COMMANDER=1` — crewborg's
  ...
  mechanism — full table in [`crewrift/crewborg/README.md`](../../../../crewrift_lab/crewrift/crewborg/README.md).
...
- **`--run` silent fallback** — the quietest failure: no/incorrect `--run` ⇒ a reference player runs,
  so the version uploads fine but a *different* policy plays. Always launch `crewrift.crewborg.coworld.policy_player`.

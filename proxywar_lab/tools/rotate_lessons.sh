#!/usr/bin/env bash
# SessionStart hook: rotate the tentative-lessons buffer (James, 2026-07-10).
#
...
BUFFER="$REPO/proxywar_lab/TENTATIVE_LESSONS.md"
ARCHIVE_DIR="$REPO/proxywar_lab/lessons_archive"
...
if [[ "$SOURCE" != "startup" && "$SOURCE" != "clear" ]]; then
  ...
  emit_context "Tentative-lessons buffer (this session's, write lessons AS YOU GO): proxywar_lab/TENTATIVE_LESSONS.md"
  exit 0
fi
...
if [[ -n "$ARCHIVED" ]]; then
  CTX="Tentative-lessons buffer rotated: previous session's lessons archived to proxywar_lab/lessons_archive/$ARCHIVED. Fresh buffer: proxywar_lab/TENTATIVE_LESSONS.md — write candidate lessons there AS YOU GO."
elif [[ -n "$SKIPPED_DUP" ]]; then
  CTX="Fresh tentative-lessons buffer: proxywar_lab/TENTATIVE_LESSONS.md (previous buffer was a byte-identical copy of an existing archive - likely restored by a git sync; not re-archived) - write candidate lessons there AS YOU GO."
else
  CTX="Fresh tentative-lessons buffer: proxywar_lab/TENTATIVE_LESSONS.md (previous buffer was empty; nothing archived) — write candidate lessons there AS YOU GO."
fi

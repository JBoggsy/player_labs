...
## Facts worth carrying forward (verified 2026-08-11)

- **Repos**: `~/coding/coworlds/ProxyWar` @ `916d13c` (main; a normal public clone —
  pull before use, hosted package runs AHEAD of the checked-in manifest);
  `~/coding/coworlds/proxywar-coworld-starter` (public starter, MIT);
  `~/coding/coworlds/ProxyWar-starter-agent` (RETIRED relay path, reference only).
- **Hosted canonical**: `proxywar 0.1.35` = `cow_1ce44ce9-42d1-4e08-a0d3-df559f9bd44e`
  (manifest hash `sha256:e126c6…`). Structured deals ON
...
- **League**: `league_cb60d526-ecfd-4836-ab3a-81fc6cf7dc42` "Proxy War".
  ...
  count (currently 16); 8-map rotation by round number, Europe quarantined;
  `episodeIndex` rotates spawn slots. Scoring: episode 1/0 (winner = 80% map control;
  territory share on timeout); ladder = win-rate EWMA, half-life 24 rounds, 5-round
  provisional gate.
- **Episode contract (16p)**: 500 decision steps × 100 turns, `max_decision_ms`
  **15000**, connect window 120 s, episode timeout 4500 s (5400 s World), no bots —
  every seat a policy, warships disabled, starting gold 200k, all seats profile
  `opportunistic`.
- **Standings (2026-08-11)**: daveey 21.67 (954 rds) » SIAN VOIDCROWN 14.72 »
  0d1novizzz 14.69 » Andre von Houck 13.80 » Jordan 13.71 » Calc 13.45 » … »
  **James Botts rank 23, 0.0000 over 1066 rounds**. Known opponent policies: relh
  `co-gas-proxywar-relhalpha:v26`, 0d1novizzz `xX_UwU_Senpai_420_Xx:v14`, docxology
  `daf-proxywar-v9:v1`, Ari Sklar `arisk-proxywar:v1`, Matt Van `mattvan:v2`.

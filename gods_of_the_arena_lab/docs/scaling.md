...
> **Currency.** Every number here is generated from an engine dump of the deployed polyworld
> `7365e4e9` (coworld 2026.9.16.3, verified 2026-09-16). **Re-verify when** `tools/deployed_ref.py`
...
> (its prose predates this verification; the figures it embeds are the regenerated ones).
> This finding is lab-internal; do not post it publicly (see `user_preferences.md`).

Research report · 2026-09-15 · for James, before designing the first James Botts policy's fight, farm, and shop logic. Every number comes from the engine's own content tables, extracted by a program compiled against the deployed polyworld `7365e4e9` and modeled in `model.py`; the figures are generated by `charts.py` from that dump. This report stays in the lab: the spell-scaling finding is not to be posted publicly.

## Executive summary

Only four hero numbers change with level: max HP, max mana, basic-attack damage, and move speed, each by a fixed per-class step (`content.nim:744-762`). Everything else is a constant for the whole match: attack speed, attack range, every ability's damage, heal, mana cost, cooldown, and range, every item bonus, every consumable, footman and tower stats, and every reward. Over 19 levels HP grows 3.9 to 4.6 times and basic damage 4.5 to 5.6 times while mana grows 2.4 to 2.9 times and move speed only 1.2 to 1.3 times, so the game's balance changes shape as heroes level. A basic-attack duel between two level-1 heroes takes 4 to 17 seconds and the same duel at level 20 takes 3 to 17, because attacker damage and defender HP grow in step. Spells do not: a level-1 ultimate removes a third to a half of an enemy hero's HP, and the same ultimate at level 20 removes 7 to 12 percent (figure 9). By level 5 to 13 every class's full ten-second ability burst has fallen below half an enemy's HP, and by then basic attacks already deliver several times the sustained damage of the whole kit (figures 11 and 12).

The same fixed numbers cut both ways. Footmen, whose 60 HP and 12 damage never change, go from a two- or three-hit kill and a real threat at level 1 to a one-hit kill and background noise by level 3 to 11 (figure 15). Towers fade more slowly but they do fade: a gate tower kills a level-1 hero in 7 to 12 seconds and a level-20 hero in 24 to 51 seconds (figure 16), while a lone hero needs about 68 seconds of basic attacks to take a gate at level 1 and 14 at level 20 (figure 17), so from roughly level 10 a full-HP hero out-lasts the tower it is hitting. A +14 damage weapon is a third to two thirds of a hero's damage at level 1 and 6 to 14 percent at level 20; an elixir heals a quarter to a half of a level-1 hero and 6 to 13 percent of a level-20 one (figures 18 to 20). Rewards are flat too, so a hero kill is worth 1.5 levels at level 1 and a tenth of a level at level 19 (figure 3). The practical shape is a three-phase game: early, where spells, potions, and cheap weapons decide fights and creeps hurt; middle, where levels and equipment take over; and late, where the only things that still scale are basic attacks and HP, every fight is a long basic-attack race decided by HP and numbers, and towers and barracks fall to whoever shows up with a wave.
...
- Four per-hero numbers grow linearly with level: HP, mana, basic damage, move speed (`content.nim:744-762`).
- Nothing else in the game has a level term: abilities, attack cadence, range, items, consumables, footmen, towers, barracks, rewards.
...
The scaling functions are four one-liners: `base + (level - 1) * perLevel` for HP, mana, damage, and move (`content.nim:744-762`). `refreshHeroStats` applies the HP and mana growth at each level-up, and `heroAttackDamage` and `heroMoveSpeed` read the damage and move values at use time with equipment added (`sim.nim:722-760`). Attack cadence and range are per-class constants with no level parameter (`content.nim:138-319`, `heroAttackTicks` and `heroAttackRange` at `content.nim:771-777`). Ability specs carry fixed `damage`, `heal`, `restore`, `manaCost`, `cooldownTicks`, `range`, and area values; the effective-spec function adds charges and recharge rules but no level term, and `hitSpellTarget` applies `spec.damage` unchanged (`content.nim:607-717`, `sim.nim:2414-2433`). Item bonuses are constants (`content.nim:512-594`); footman HP and damage, tower HP, damage, and range, and the three reward pairs are constants in the simulation (`sim.nim:259-266`, `sim.nim:369-370`, `sim.nim:387-392`). Barracks reuse the outer-tower HP value, 950 (`sim.nim:3520-3528`).
...
- XP to leave level L is `100 + (L - 1) × 75`; level 20 needs 14,725 XP (`sim.nim:699-701`).
- With one fifth of the team's creeps a hero ends a 20-minute match at level 12; with a whole lane, level 15; a lane plus a hero kill every two minutes, level 16.
- Only a hero taking every creep on the map reaches level 20, at about minute 11.

Figure 2 shows the curve and four income scenarios (footman 25 XP; each lane has two enemy barracks that spawn 3 creeps each, 18 enemy footmen per team per 480-tick wave, `sim.nim:1330-1373`, `content.nim:10`, `src/polyworld/configs.nim:11`). The scenarios are rates, not plans: 4.5 XP/s is an even five-way split of the team's creeps, 7.5 XP/s is every last hit in one lane, 8.75 XP/s adds one hero kill (150 XP) every 120 seconds, and 22.5 XP/s is the theoretical ceiling of all three lanes. The supply is not fixed: a destroyed barracks stops its 3 creeps per wave for the rest of the match (section 6), so a lane that loses both barracks stops paying. The realistic band for a policy that farms well but shares is level 12 to 16 by the time limit. Levels 17 to 20 exist in the tables but a match rarely reaches them, so the "late" phase in this report is roughly levels 12 to 16, not 20.
...
The full per-level table is Appendix A. Two things to read off the panels. First, the spread between the best and worst basic DPS grows only from 2.4× at level 1 to 2.7× at level 20 (Demon Hunter against Druid Warden) and the ordering changes only once (Arcanist passes Crossbowman at level 2), so class matchups do not flip with level. Second, because HP and damage both grow, the ratio of a hero's own DPS to its own HP is nearly flat: a hero can always kill a copy of itself in roughly the same number of seconds (section 4).
...
- The fastest killers are Demon Hunter and Berserker (3 to 8 seconds against anything); the slowest are Druid Warden and Warlock (8 to 17 seconds).
...
The average enemy HP is the mean over the five opposing classes (Appendix B). At level 1 the Red team's ultimates are the largest single hits in the game: Lich's Bound Void (125) is 53 percent of an average Blue hero, Crossbowman's Clockwork Charge (115) 48 percent, Death Knight's Dark Eclipse (110) 46 percent. Blue's best is Arcanist's Arcane Meteor (120) at 46 percent of an average Red hero. The primary strikes (32 to 50 damage, three charges) are 12 to 21 percent each at level 1, so three charges in a row is most of a squishy target. By level 10 the ultimates are 13 to 20 percent and the primaries 5 to 8 percent, and by level 20 every ability is single digits except the five biggest ultimates at 10 to 12 percent.

The heal side (figure 10) has the same shape. Druid Warden's Kindred Wisps (80) heals a third of its own level-1 HP and 7 percent at level 20; the self-heal passives (18 to 36) are 7 to 10 percent of own HP at level 1 and 1 to 2 percent at level 20. Since these fire automatically whenever the hero is below max, their late-game effect is a trickle.
...
- A footman (60 HP) takes 2 or 3 basic hits at level 1 and 1 hit from level 3 (Crossbowman) to level 11 (Druid Warden) on (figure 15).
- A gate tower kills a hero in 7 to 12 seconds at level 1 and 24 to 51 seconds at level 20 (figure 16); footman damage is 4.8 percent of average hero HP at level 1 and 1.1 percent at level 20.
- A lone hero needs 33 / 45 / 68 seconds of basic attacks for an outer / inner / gate tower at level 1, 12 / 16 / 24 at level 10, and 7 / 9 / 14 at level 20 (figure 17). A barracks has outer-tower HP (950) and pays the tower reward.
...
Figure 16 — Seconds a hero survives standing under a gate tower (30 damage per second) with no healing.
...
Footmen stop mattering as a threat around level 5 (their 12 damage is 4 percent or less of any hero's HP) and stop costing time between level 3 and level 11 (one hit each), but they never stop being the economy: 25 XP and 15 gold each, unchanged. Towers are fixed on both sides, HP and damage, so a hero's time to take one falls with level while the tower's time to kill the hero rises. The two curves cross: at level 1 an average hero survives a gate tower for about 8 seconds and needs 68 to kill it, at level 10 it survives 22 seconds against a 24-second kill, and at level 20 it survives 37 seconds against 14. Sieging a gate solo needs creeps to absorb tower fire (towers target footmen before heroes, `sim.nim:1942-2018`) until about level 7 (Berserker) to 14 (Druid Warden, Lich); an outer tower can be taken alone from level 2 to 6. With a wave in front, the hero's time to kill the tower is the binding number at every level, and that is what levels buy.

Behind the three towers each lane holds two barracks per team, objects of kind 5 with 950 HP each (`sim.nim:3520-3528`). A barracks cannot be attacked while any tower in its lane stands (`sim.nim:655-663`), and it does not shoot back. Killing one pays the tower reward, 100 XP and 75 gold (`sim.nim:2264-2268`), and stops its 3 creeps per wave (`sim.nim:1330-1335`), which is a third of that lane's income for the other team. At 950 HP it costs a hero the same time as an outer tower: 33 seconds of basic attacks at level 1 and 12 at level 10, averaged over the classes.

## 7. Items and consumables against level

- +14 damage (Battle Axe or Rune Crossbow, 180 gold) is 30 to 64 percent of basic damage at level 1 and 6 to 14 percent at level 20 (figure 18).
...
- The game has three phases keyed to level, not clock: burst (levels 1 to about 5), transition (about 5 to 10), and attrition (10 and up). With the current creep supply a hero on a fifth of the team's creeps reaches level 10 near minute 13 and ends the match at level 12; a full lane reaches 10 near minute 8 and ends at 15.
...
- Fight early when your burst is above the enemy's HP share and theirs is not; fight late only with an HP or numbers advantage, because there is no burst to swing it. Take towers and barracks with a wave until about level 10; after that a full-HP hero can take one alone.
...
**Transition (levels 5 to 10).** Burst falls through 50 percent, footmen become one-hit kills for most classes, and the +14 weapon is still 17 to 37 percent of damage at level 5 and 11 to 24 at level 10. Fights start to depend on who has more HP and allies. This is where equipment matters most and where a policy should be converting gold as fast as it comes in.

**Attrition (level 10 and up).** Ultimates are 9 to 20 percent of an enemy, sustained kit damage is a tenth of basic DPS, and time to kill in a duel is unchanged from level 1 while there is nothing to shortcut it. Towers stop being lethal: a gate tower takes 15 to 43 seconds to kill a level-10 to level-16 hero and falls to that hero alone in about 17 to 24 seconds, so the objective game becomes a race to clear a lane's three towers and two barracks. This is a game of HP pools, creep waves to speed up sieges, and picking fights at numbers advantage. Heals are a trickle, so retreating to recover is about respawn timers and potions, not abilities.
...
- `examples/gods_of_the_arena/sim.nim` — level curve, stat refresh, attack cadence, cast and charge mechanics, mana regeneration, tower, barracks, and footman constants, wave spawning, rewards, purchase rules.
- `src/polyworld/configs.nim` — default match length and spawn interval.
- `gods_of_the_arena_lab/tools/scaling/dump_specs.nim`, `specs.json`, `model.py`, `charts.py` — the extraction, model, and figure code for this report.

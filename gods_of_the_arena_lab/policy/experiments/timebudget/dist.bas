...
sub cfgInit()
  ...
  ' Classes that farm better by standing at the standoff point and letting the
  ' engine auto-attack than by holding for windows: low-damage or melee heroes
  ' (Vanguard Knight, Druid Warden, Death Knight, Warlock). Measured locally
  ' (Codex variant B, 2026-09-17): +43, +61, +23, +14 last hits per match for
  ' those classes; Ranger, Demon Hunter and Crossbowman lose with idle.
  cfgIdleHere = 1
...
'   ehN, ehX(i), ehY(i), ehTgt(i)              visible living enemy heroes and their targets
        ...
        if objectAlive(scI) = 1 then
          if scTeam = selfTeam then
            if ahN < 5 then
              ...
              ahX(ahN) = objectX(scI) * 60
              ahY(ahN) = objectY(scI) * 60
    ...
    if ahTgt(laI) >= 1000 then
      ...
      if laJ >= 0 then
        if efId(laJ) = ahTgt(laI) then
          efHero(laJ) = ahDmg(laI) * 256 + ahPer(laI)
        end if
      end if
    end if
    laI = laI + 1
    ...
    if liEv > 0 then
      ...
      if worldTick - liT <= cfgEventStale then
        ...
        if efTw(i) > 0 and liA = efTw(i) then
          ...
          if efHero(i) / 256 > 0 and liA = efHero(i) / 256 then
            liPer = efHero(i) mod 256
          else
...
sub svPlan()
  ...
  while svI < ehN
    ...
    svD2 = svDx * svDx + svDy * svDy
    if ehTgt(svI) = selfId and svD2 <= cfgHeroHuntRange * cfgHeroHuntRange then
      svThreat = 1
    end if
    if selfHp * 100 < selfMaxHp * 60 and svD2 <= cfgHeroFearRange * cfgHeroFearRange then
      svThreat = 1
    end if
    svI = svI + 1
  wend
...
if mnActed = 0 then
  ...
  mnChase = 0
  if cfgChaseClass = 1 and lhState = 1 and lhAnchor >= 0 then
    mnDx = efX(lhAnchor) - scSelfX
    mnDy = efY(lhAnchor) - scSelfY
    if mnDx * mnDx + mnDy * mnDy <= lhChase * lhChase then
      mnChase = 1
    end if
  end if
  if mnChase = 1 then
  ...
  if cfgIdleHere = 1 and lhState = 1 and mnIdleDx * mnIdleDx + mnIdleDy * mnIdleDy <= 1 then
...
sub mnLaneFront()
  ...
  if mnBest >= 0 then
    ...
    mnI = 0
    while mnI < ehN
      mnDx = ehX(mnI) - afX(mnBest)
      mnDy = ehY(mnI) - afY(mnBest)
      if mnDx * mnDx + mnDy * mnDy <= cfgHeroFearRange * cfgHeroFearRange then
        mnUnsafe = 1
      end if
      mnI = mnI + 1
    wend
    if mnUnsafe = 1 then
      mnI = 0
      while mnI < ahN
        mnDx = ahX(mnI) - afX(mnBest)
        mnDy = ahY(mnI) - afY(mnBest)
        if mnDx * mnDx + mnDy * mnDy <= cfgHeroFearRange * cfgHeroFearRange then
          mnUnsafe = 0
        end if
        mnI = mnI + 1

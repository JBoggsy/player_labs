...
mnActed = 0
if svRetreat = 1 then
  if selfTarget <> 0 and selfAttackCooldown < lhWindup then
    ' Literal variant C: even retreat waits for an already committed swing.
    mnActed = 1
...
if mnActed = 0 and selfTarget <> 0 and selfAttackCooldown < lhWindup then
  ' Preserve a committed swing when the fallback would otherwise walk away.
  ' The retreat branch applies the same committed-swing condition.
  mnActed = 1
  mnVariantActs = mnVariantActs + 1
end if
if mnActed = 0 then

...
### An LLM prompt that attaches a field to ~every response makes that field worthless as a confidence signal — gate on the ACTION, not the payload
Evidence: v88's vote gate accepted any LLM-named target as corroboration, but the v87 prompt rides `vote_target` on ~every send_chat, so the gate fired 0/35 league eps and the LLM-named-tentative clause fed 10 wrong vs 3 right votes. v89 tightened corroboration to the LLM's explicit submit_vote ACTION only → gate fires 17/50, crew vote precision 36%→71% (p=0.002), mis-ejections fed 7→2, imposters-ejected unchanged. When a model can attach a field opportunistically, only a decision that COMMITS to it (the action) carries signal.
### belief self-state flags need a meeting-census backstop — HUD icons can skip a whole phase transition
Evidence: `self_alive` was only cleared by the ghost HUD icon; a seat killed just before a meeting (ep 422637ce: killed t=1070, meeting t=1142) never rendered it, played the meeting as "alive", and leaked 4 dead-seat LLM calls past the v88 mute. The vote-screen candidate grid (census) is authoritative and already recorded the death on the roster — mirroring it into `self_alive` in update_belief (which runs before mode.decide) closed the leak with a 5-line change. Generalization: any belief flag latched from a transient HUD signal should also latch from the authoritative periodic census when one exists.
### Matched A/B arms can still see asymmetric Bedrock quota pressure — check llm_call_failed per arm before attributing deltas
Evidence: v89/v88 50v50 back-to-back arms: v89 ate 294 llm_call_failed vs v88's 175 (decisions 124 vs 261) despite identical recipes and overlapping windows. Here the bias ran AGAINST the winning arm so the verdict stands a fortiori; had v89 lost, this would have been a confound requiring a re-run. Count `meeting_llm_fallback` reason=llm_call_failed per arm as a standard A/B sanity column.
### Judge a version against its WINDOW's field par, not the previous version's raw number
Evidence: v89 league crew 24% looked like a collapse vs v88's 43% — but the field's crew-team win dropped 37%→25% in the same window (imposter-draw rotation + notsus v159 + field first-kills ~900 ticks faster), and v89's 24% = exactly field par AND statistically identical to its own A/B arm (p=0.78). v88's 43% was the outlier (+6pp over ITS window par, draw-lucky +2.2pp). Every crew-heavy policy collapsed together (forgeling 41→14%, softmaxwell 39→7%). Always compute field-par delta before calling a regression.

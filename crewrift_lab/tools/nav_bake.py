...
bakes them ONCE into the vendored asset `crewrift/crewborg/map/croatoan_navbake.pkl.gz`,
...
1. CAPTURE the authoritative walkability mask crewborg actually sees. Run a local
   ...
   bit-packed JSON line to its stderr / policy log), then extract it:
       ...
       tools/nav_bake.py extract-walkability \
           /tmp/coworld_smoke/logs/policy_agent_0.log -o /tmp/croatoan_walkability.npy

2. BAKE the asset from that mask:

       tools/nav_bake.py bake /tmp/croatoan_walkability.npy
       # -> writes crewrift/crewborg/map/croatoan_navbake.pkl.gz (+ prints per-stage timing)

Then rebuild the image (tools/build_player.sh crewborg) so the asset ships, and the
...
from crewrift.crewborg.agent_tracking import build_occupancy_substrate
from crewrift.crewborg.map import load_croatoan_map
from crewrift.crewborg import nav as nav_mod
from crewrift.crewborg.navbake import NAVBAKE_PACKAGE, NAVBAKE_RESOURCE, serialize_navbake


def _navbake_asset_path() -> Path:
    return Path(__file__).resolve().parents[1] / "crewrift" / "crewborg" / "map" / NAVBAKE_RESOURCE

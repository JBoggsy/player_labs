...
BUFFER="$REPO/crewrift_lab/TENTATIVE_LESSONS.md"
ARCHIVE_DIR="$REPO/crewrift_lab/lessons_archive"
...
record_state() {  # let the Stop nudge detect "buffer untouched this session"
  { md5 -q "$BUFFER" 2>/dev/null || md5sum "$BUFFER" 2>/dev/null | cut -d' ' -f1; } \
    > "$STATE_DIR/claude_lessons_baseline_$SESSION_ID" 2>/dev/null || true
}
...
if [[ "$SOURCE" != "startup" && "$SOURCE" != "clear" ]]; then
  ...
  [[ -f "$STATE_DIR/claude_lessons_baseline_$SESSION_ID" ]] || record_state
  emit_context "Tentative-lessons buffer (this session's, write lessons AS YOU GO): crewrift_lab/TENTATIVE_LESSONS.md"
  exit 0
fi
...
if [[ -n "$ARCHIVED" ]]; then
  CTX="Tentative-lessons buffer rotated: previous session's lessons archived to crewrift_lab/lessons_archive/$ARCHIVED. Fresh buffer: crewrift_lab/TENTATIVE_LESSONS.md — write candidate lessons there AS YOU GO (a Stop hook will nudge once if substantive work happens with the buffer untouched)."
else
...
if [[ -n "$ARCHIVED" ]]; then
  git -C "$REPO" add "$BUFFER" "$ARCHIVE_DIR" >/dev/null 2>&1 \
    && git -C "$REPO" commit -q -m "lessons: rotate session buffer -> lessons_archive/$ARCHIVED (SessionStart hook)" \

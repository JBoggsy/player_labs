...
BASELINE_FILE="$STATE_DIR/claude_lessons_baseline_$SESSION_ID"
...
touch "$MARKER"
jq -n '{decision: "block",
        reason: "Lessons check (automated, fires once per session): this session did substantive work but crewrift_lab/TENTATIVE_LESSONS.md is untouched. Before stopping, either (a) add the session'\''s candidate tentative lessons to that buffer — eagerly, noise is fine — or (b) if you judge there are genuinely none, append a one-line entry saying so with a short justification. Then finish your reply."}'

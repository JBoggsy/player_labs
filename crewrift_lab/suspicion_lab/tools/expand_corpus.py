...
binary and re-run with --ref).
    ...
    uv run python crewrift_lab/suspicion_lab/tools/expand_corpus.py            # default ref 42fed21
    uv run python crewrift_lab/suspicion_lab/tools/expand_corpus.py --ref <sha> --workers 8
"""
...
def expander_path(ref: str) -> Path:
    ...
    if not path.exists():
        sys.exit(
            ...
            f"Build it: crewrift_lab/tools/build_expand_replay.sh --ref {ref}"
        )
    return path

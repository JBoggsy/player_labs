...
# Logs:   crewrift_lab/suspicion_lab/logs/nightly-<date>.log
set -uo pipefail

REPO="/Users/jamesboggs/coding/personal_labs"
export PATH="$HOME/.local/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin"

SUSPICION_LAB="$REPO/crewrift_lab/suspicion_lab"
WEIGHTS_DST="$REPO/crewrift_lab/crewrift/crewborg/data/suspicion_weights.json"
VERSION_LOG="$REPO/crewrift_lab/crewrift/crewborg/version_log.md"
...
# --- 1. scrape + expand ----------------------------------------------------------
step "scrape" uv run python crewrift_lab/suspicion_lab/tools/scrape_corpus.py --max-rounds 150
step "expand" uv run python crewrift_lab/suspicion_lab/tools/expand_corpus.py --workers 8

# --- 2. dataset + fit -------------------------------------------------------------
step "dataset" uv run python crewrift_lab/suspicion_lab/tools/build_dataset.py
step "fit" uv run python crewrift_lab/suspicion_lab/tools/fit.py --tag "$TAG" --features runtime
...
if ! uv run pytest crewrift_lab/crewrift/crewborg/tests -q; then restore; abort "test suite failed on new weights"; fi

step "build" crewrift_lab/tools/build_player.sh crewborg
if ! uv run python .claude/skills/coworld-local-run/scripts/smoke.py \
      --coworld crewrift --image players-crewborg:dev \
      --run python --run=-m --run crewrift.crewborg.coworld.policy_player --timeout 240; then
...
path = "crewrift_lab/crewrift/crewborg/version_log.md"

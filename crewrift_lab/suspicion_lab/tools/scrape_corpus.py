...
Usage (auth from `softmax login`; run from the player_labs repo root):
    ...
    uv run python crewrift_lab/suspicion_lab/tools/scrape_corpus.py            # default: Crewrift/Competition
    uv run python crewrift_lab/suspicion_lab/tools/scrape_corpus.py --max-rounds 10
    uv run python crewrift_lab/suspicion_lab/tools/scrape_corpus.py --division div_...
...
LAB_ROOT = Path(__file__).resolve().parents[2]          # crewrift_lab/

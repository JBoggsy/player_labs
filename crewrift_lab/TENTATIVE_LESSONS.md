...
### TOURNAMENT Bedrock 403s the SAME as experience requests — tournament-works hypothesis REFUTED; v49 rejected from Prime (2026-06-26)
James submitted v49 (LLM meetings, --use-bedrock + forced USE_BEDROCK, NO SDK sidecar fix) to Crewrift Prime to test whether tournament pods reach Bedrock differently than experience-request pods. RESULT: submission `sub_0dc3e42c` = **REJECTED** (didn't qualify; played ~10 qualifier rounds, 9 clean/1 ops). v49 league telemetry: **34/34 meeting_llm_fallback, reason=llm_call_failed, error=403 "security token invalid"; ZERO meeting_llm_decision** — IDENTICAL to the experience-request failure. So tournament ≠ working Bedrock for our client; both hit the same 403 (client targets AWS directly via AnthropicBedrock, bypassing the loopback sidecar). The hypothesis that tournament jobs proxy Bedrock differently is REFUTED — the SDK fix (coworld-tools PR #12: pass base_url from AWS_ENDPOINT_URL_BEDROCK_RUNTIME) is required in BOTH contexts, plus the infra sidecar-enablement for crewrift_prime. NB tension with the old "CnW tournament-pod Bedrock VERIFIED working" memory — for crewborg-now, tournament 403s; either the platform changed (sidecar rollout) or that prior verification wasn't a true hosted tournament. REJECTION cause: v49 mean seat score only 0.7 (mostly crewmate task 6-8, dragged by 2× -100 crashes) — below the qualification bar. POSSIBLE (verify, don't assert): the LLM makes a FAILING Bedrock network round-trip every meeting; that latency/hangs may relate to the -100 crashes / low score. If we want v49's deterministic search gains in the league, resubmit with CREWBORG_LLM_MEETINGS off (deterministic meetings) until Bedrock end-to-end works.

### Expander: crewrift_prime 0.4.3 THROUGH 0.4.7 all use the SAME sim (master 26ee08c = /tmp/expand-043) — no rebuild needed (2026-06-26)
v50 episodes ran on crewrift_prime **0.4.7** (Prime keeps bumping: 0.4.3→0.4.4/0.4.5→0.4.7). Expected to need a new expander, but: master sim is UNCHANGED since 26ee08c (`git diff 26ee08c..3a0406e -- src/crewrift/*.nim` = empty; the 4 newer master commits are docs/vendoring only), AND **expand-043 (26ee08c) expands all 8 sampled v50 0.4.7 replays FULLY to GameOver, 0 hash-fails (incl. a button game)**. So the prime fork's 0.4.3→0.4.7 version bumps did NOT change the sim hash logic — `/tmp/expand-043` is the correct expander for 0.4.3 through 0.4.7. Use `CREWRIFT_EXPAND_REPLAY=/tmp/expand-043` for the warehouse on current Prime episodes. METHOD reminder: don't assume a version bump = sim change; test the existing expander on the new replays first (trace_complete to GameOver + 0 hash-fails, button game included).

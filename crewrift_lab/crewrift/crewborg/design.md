...
| Mode | Active when | Intents emitted |
|---|---|---|
| **Search** | the imposter's **always-on seeking stance** (default when not evading / reconning / hunting; Pretend retired 2026-06-24) | **go to the room holding the most recently-seen crew** (current room included — so after a kill we re-approach the crew still nearby instead of wandering; falls back to a random nearby room only when no crew are known — added 2026-06-25 from the post-kill diagnosis: Search's old random room-pick left us ~150px from crew while Aaron closed onto them), watch from the max-LOS vantage, and when a crewmate **leaves** follow them via **path prediction** (`strategy.path_prediction`). Keeps us *near crew* so a kill window opens (Hunt takes over) |

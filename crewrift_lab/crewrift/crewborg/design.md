...
| **Recon** | not ready, a fresh sighting exists, **and** the real travel time to it has caught up with the remaining cooldown (reworked 2026-07-06 — was a fixed `recon_window()` tick count) | **beeline to the most ISOLATED fresh non-teammate crewmate** (`strategy.opportunity.most_isolated_recon_candidate`, via `modes/recon.py`; live position when visible, last-known otherwise) so a victim is in hand the instant we can kill → Hunt fires immediately, timed to arrive right as the cooldown clears. Built 2026-06-25 from the warehouse finding that we had a crew in view *at* cooldown-ready only 53% of the time vs Aaron's 83%; the fixed-window version risked the over-extension that gets an early-committing imposter caught, so the trigger is now computed (`travel_ticks`) instead of a flat tick count |
...
| **WATCH**(room) | hold the in-room **task station with line-of-sight to the most crew** — never an arbitrary room point — recomputed (throttled, with hysteresis) as they move, so we keep crewmates in sight rather than standing at the entrance and letting them walk out of view. LOS via `nav._segment_clear` over the walkability mask, within `VANTAGE_RANGE` | a watched crewmate **leaves** the room → **FOLLOW**(them); no watched crew still around → **PICK_ROOM** |
...
4. not ready, a fresh sighting exists, and travel time to it now matches the
   remaining cooldown → **Recon** (beeline to the most isolated fresh crewmate,
   timed to arrive as the cooldown clears)
...
| LLM meetings | opt-in with `CREWBORG_LLM_MEETINGS=1` plus Bedrock (`USE_BEDROCK=1`) or `ANTHROPIC_API_KEY`; default models `claude-haiku-4-5-20251001` direct / `us.anthropic.claude-haiku-4-5-20251001-v1:0` Bedrock; default meeting timeout 6.0s (`CREWBORG_LLM_MEETING_TIMEOUT_SECONDS`; raised from 3.0s 2026-07-21 — at success-latency p90 ~4.0s the old timeout aborted ~40% of successes into token-double-spending SDK retries); calls run on a background worker (never block the loop); ≥120 ticks between calls (`CREWBORG_LLM_MIN_CALL_INTERVAL_TICKS`) and ≤`CREWBORG_LLM_MEETING_CALL_BUDGET` (default 5) calls per meeting; deadline prompt wins over chat prompts; fallback vote = own-chat accusation → `top_suspect` → skip; tentative non-skip votes early-submit once the budget is spent or <50% believed time remains, else auto-submit in the ≤48-tick window; chat cooldown is `CHAT_COOLDOWN_TICKS = 60` |
...
| Kill witness gate | unwitnessed iff the count of OTHER live non-teammate crewmates currently visible to us THIS tick is at or below the urgency-ramped tolerance (vision is symmetric, so "visible to us" == "could see the kill" — no radius/staleness proxy needed, see `docs/designs/vision-model.md`); tolerance ramps from `ALLOWED_WITNESSES_MIN = 1` at zero urgency (a lone onlooker never vetoes) to `ALLOWED_WITNESSES_MAX = 6` (this game's fixed 6-crew format means at most 5 others can ever witness, so 6 is an "always strike" ceiling) by `URGENCY_FULL_TICKS = 240` kill-ready ticks, **and skipped entirely after our first kill** (Hunt's strike gate bypasses `unwitnessed` once `last_kill_tick` is set — prioritize banking the 2nd kill over stealth) |

...
**Location:** `~/coding/players/players/player_sdk` (the `players` repo, installed
editable into the lab — the SDK is imported, not vendored in this fork).
...
`nottoodumb`) with guides `how_to_make_a_bot.md`, `SMART_BOT_GUIDE.md`,
...
- **`players/cogsguard/{baseline,buggy,cranky}`** — JSON/`coworld_json_bridge`
  players (token-grid game). **Not** crewborg's transport, but a reference for the
  SDK's JSON bridge.
...
Absolute roots:
- Player SDK: `~/coding/players` (pkg `players`, installed editable into the lab)

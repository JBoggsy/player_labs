...
| **ReconMode** (`modes/recon.py:decide`, `:_commander_target`) | `recon_target` (most isolated fresh sighting) | close on `target_player` if it is alive, known, and not a teammate; else `recon_target` | (same) |

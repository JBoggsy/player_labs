...
1. **Resolve the roster by pinning champions, not `top_n`/`random`.** Pin exact identities for reproducible comparisons.
...
2. **Split anything over 100 episodes into sequential requests.** `num_episodes` caps at 100/request
   under the current API contract. Create N requests via `coworld-experience-requests`' `experience_request.py create`, one per 100-episode chunk.
3. **Stream artifacts with `--no-artifacts --no-logs`** unless the study genuinely needs per-agent
   telemetry zips or policy logs. Omit unneeded large downloads. `--watch` is crash-safe/resumable either way (relaunch the same command).
...
7. **Spot-check a suspiciously uniform aggregate before reporting it.** A flat 0.0% (or 100%)
   across every row is this lab's standing tell for a tooling artifact, not a real finding — check event ordering and joins (`replay_parse.py`'s per-meeting `ejected_slot` silently never set,

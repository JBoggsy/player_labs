...
| [telemetry-harvest.md](./telemetry-harvest.md) | The league telemetry harvest — `tools/harvest_artifacts.py` (idempotent, cron-able puller of crewborg's league `policy_artifact_*.zip` bundles into `telemetry_harvest/`), the crontab line, and the resolved 2026-07 "artifacts are ephemeral" retention scare (they're durable; the July-1 disappearance was a read-path/auth artifact, with metta refs). |

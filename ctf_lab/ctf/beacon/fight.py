...
where beacon moves, while this module chooses which *visible* enemy the gun
watches. The score favors finishable wounds, the effective 220-300px gun band,
clear firing geometry, and low traverse cost. HP labels mirror
``labels.nim``: ``hp <lit>/<total>`` is an ordinal bar over HP plus shield, not
hit-point arithmetic. Shields break under ``sim.nim`` GameVersion 23, so their
visible carried marker is a small independent penalty.

The score is only *largely* shared. Wounds and enemy positions overlap between
bots, but range, aim cost, visibility/fog, and friendly-fire corridors are
shooter-local.
Focus claims are therefore load-bearing convergence machinery, not decorative
tie-break metadata: they correct similar local rankings toward one target while
remaining a bounded bias rather than an order.

Runtime ranking uses the shipped 32-direction sightline field for O(1) bullet
line checks over at most eight enemies. The field includes glass because glass
blocks movement, bullets, and plasma in ``bake_map.py``. action.py retains the
exact ``mapdata.ray_clear`` check for the one selected target before firing.
"""
...
from ctf.beacon import mapdata, squads
...
def baked_line_clear(
    origin: tuple[int, int],
    target: tuple[int, int],
) -> bool:
    """Cheap bullet-line estimate from the shipped sightline field.

    The field is sampled from nav-cell centres, while live actors may be up to one
    cell away; one ``NAV_CELL`` of tolerance prevents that quantization seam from
    declaring an otherwise open target blocked. The chosen target still passes
    action.py's exact ray check.
    """
    ...
    direction = round(
        angle / (2 * math.pi) * SIGHTLINE_DIRECTIONS
    ) % SIGHTLINE_DIRECTIONS
    gx = min(max(origin[0] // NAV_CELL, 0), GRID_W - 1)
    gy = min(max(origin[1] // NAV_CELL, 0), GRID_H - 1)
    free_px = (
        int(mapdata.sightline_field()[direction, gy, gx])
        * SIGHTLINE_DISTANCE_UNIT_PX
    )
    return distance <= free_px + NAV_CELL
...
def _visible_target(
    ...
    matches = [
        enemy
        for enemy in belief.enemies
        if _enemy_matches_ref(belief, enemy, target)
    ]
    if not matches:
        return None
...
def _record_selected_target(belief: Belief, selected: TargetScore) -> None:
    ...
    changed = (
        belief.firefight_target is not None
        and not _refs_match(belief, belief.firefight_target, target)
    )
    if changed:
...
def select_target(
    ...
    scored = sorted(
        (
            score_target(
                candidate,
                claimed=_claimed_target(belief, candidate.target),
            )
...
def receive_focus_claim(
    ...
    *,
    ...
    if (
        current is not None
        and belief.tick - current.refreshed_tick > FF_CLAIM_TTL_TICKS
    ):
    ...
    if current.first_tick == belief.tick:
        current_rank = (squads.rank_of(current.claimant_seat), current.claimant_seat)
        incoming_rank = (squads.rank_of(claimant_seat), claimant_seat)
...
def focus_claim_to_send(belief: Belief) -> TargetRef | None:
    ...
    if (
        ...
        or belief.tick - belief.focus_last_claim_sent_tick
        < FF_CLAIM_REBROADCAST_TICKS
    ):
        return None
    ...
    if claim is None:
        return target
    if (
        claim.claimant_seat == belief.seat
        and _refs_match(belief, claim.target, target)
    ):
        return target
...
def _update_claim_lifecycle(belief: Belief) -> None:
    ...
    if claim is None:
        return
    # The claimant may have died outside our view. Expire on the communication
    # clock even while the target remains visible, or a dead claimant can retain
    # exclusive ownership forever.

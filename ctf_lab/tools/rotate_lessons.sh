...
if [[ -n "$ARCHIVED" ]]; then
  ...
  CTX="Fresh tentative-lessons buffer: ctf_lab/TENTATIVE_LESSONS.md (previous buffer was a byte-identical copy of an existing archive - likely restored by a git sync; not re-archived) - write candidate lessons there AS YOU GO."
else

...
**Announce at start:** "Reviewing the CTF lessons archive — clustering
recurring lessons across N session buffers."

## Inputs

- `ctf_lab/lessons_archive/*.md` — one buffer per past session (rotated
  automatically by the SessionStart hook). `lessons_archive/reviewed/` holds
  already-reviewed buffers — exclude them.
- `ctf_lab/TENTATIVE_LESSONS.md` — the live buffer; include it read-only
  (it stays in place; it is NOT retired by this review).
- `ctf_lab/best_practices.md` + root `best_practices.md` — the graduation
...
4. **Apply the decisions:** graduated lessons → `ctf_lab/best_practices.md`
   (CTF-specific) or root `best_practices.md` (game-agnostic), rewritten as

## Emerg-ant GV52 protocol constants and environment-backed Stencil tuning.
##
...
const
  ...
  # GV52 uses continuous integer-brad aim and turns five brads per held
  # B/Select tick; there is no slot grid.
  AimTurnRate* = 5
  AimTurnDeadbandBrads* = AimTurnRate div 2
  AimSweepStepBrads* = AimTurnRate
  # `visionConeDeg` is the half-angle around aim in the owner engine.
  GuaranteedVisionConeHalfDeg* = 60
  VisionBubble* = 90
  FireWindupTicks* = 5
  ...
  ArcIdealRangePx* = 100
  # Emerg-ant GV52 retains the 170px spray cone and 17px body-radius test.
...
let
  ...
  EarlyDefense* = envTunableBool("STENCIL_EARLY_DEFENSE", false)

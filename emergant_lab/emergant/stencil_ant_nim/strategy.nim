...
proc makeIntent*(kind: IntentKind, point: Option[Point], reason: string): Intent =
  ...
  of "raid_cache":
    ...
    result.micro.incl(MicroRaidRushExempt)
  ...
  of "intercept_raider", "intercept_raider_heard":
    result.movingGoal = true
...
proc raidGoal(belief: Belief): Option[Point] =
  if belief.worldmap.isNil or belief.selfXy.isNone or belief.raidTarget.isNone:
    return none(Point)
  let target = belief.raidTarget.get
  if belief.foods.hasKey(target):
    let food = belief.foods[target]
    if food.atCache and food.pos.isSome:
      return belief.reachableGoal(food.pos.get)
  belief.reachableGoal(belief.worldmap.cache(target))
...
proc itemAnchor(belief: Belief): tuple[pos: Point, kind: string] =
  ...
  let raid = belief.raidGoal
  if raid.isSome: (raid.get, "role") else: (belief.worldmap.center, "role")
...
proc decideBaseObjective(belief: Belief): Objective =
  ...
  if belief.iCarryFoodOf.isSome:
    let home = map.capturePoint(belief.team)
    return navigate(home, "carry_home")

  if belief.ownFoodStolen:
    if belief.ownFoodRaiderPos.isSome:
      return navigate(belief.ownFoodRaiderPos.get, "intercept_raider")
    if belief.raiderFix.isSome:
      return navigate(belief.raiderFix.get.pos, "intercept_raider_heard")
  ...
  if belief.role == Forager and belief.iCarryFoodOf.isNone:
    for color in belief.colors:
      if color == belief.team or not belief.foods.hasKey(color):
        continue
      let food = belief.foods[color]
      if not food.atCache and food.carriedPos.isSome:
        return navigate(food.carriedPos.get, "escort_carrier")
  ...
  navigate(raid.get, "raid_cache")

proc decideObjective*(belief: Belief): Objective =
  let base = belief.decideBaseObjective()
  if belief.iHaveArc and belief.iCarryFoodOf.isNone and

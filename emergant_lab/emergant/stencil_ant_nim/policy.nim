...
proc decide*(policy: StencilPolicy, client: ProtocolClient): Command =
  ...
  if not policy.rolesAssigned and not policy.belief.worldmap.isNil:
    ...
    policy.belief.defensivePostFoodDistance = 0
    ...
    if policy.belief.role == Defender:
      ...
      if assignment.isSome:
        ...
        let
          food = policy.belief.worldmap.cache(policy.belief.team)
          enemyHome = policy.belief.worldmap.homeCenter(selected.opponent)
        policy.belief.defensivePostFoodDistance = pyRound(
          policy.belief.worldmap.routeDistance(selected.post.pos, food))
        ...
        policy.belief.defensivePostForward =
          ...
          policy.belief.worldmap.routeDistance(food, enemyHome)
      else:
  ...
  policy.belief.objectiveTicks.inc(objective.intent.reason)

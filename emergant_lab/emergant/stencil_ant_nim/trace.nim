...
proc roleName(role: Role): string =
  if role == Forager: "forager" else: "defender"
...
proc navigationMap(map: WorldMap): JsonNode =
  ...
  for index in 0 ..< map.teams:
    ...
    if map.caches.hasKey(color):
      entry["cache"] = pointJson(map.caches[color])
    teams.add(entry)
  var atlas = newJArray()
...
proc snapshot(policy: StencilPolicy, command: Command): JsonNode =
  ...
  var pheromones = newJArray()
  for mark in belief.pheromones:
    pheromones.add(%*{
      "pos": pointJson(mark.pos),
      "team": teamName(mark.color),
      "kind": (if mark.food: "food" else: "scout"),
    })
  ...
  result = %*{
    ...
    "defensive_post_food_distance": belief.defensivePostFoodDistance,
    ...
    "micro": belief.micro,
    "carrying": teamJson(belief.iCarryFoodOf),
    "raid_target": teamJson(belief.raidTarget),
    "own_food_stolen": belief.ownFoodStolen,
    "pheromones_visible": pheromones,
    ...
    "target_food_distance_px": (if targetScore.isSome and
        classify(targetScore.get.foodDistancePx) != fcInf:
      %rounded4(targetScore.get.foodDistancePx) else: newJNull()),
...
proc counters(policy: StencilPolicy): JsonNode =
  ...
  %*{
    ...
    "objective_ticks": countJson(b.objectiveTicks),
...
proc record*(trace: TraceState, policy: StencilPolicy, command: Command) =
  ...
  try:
    ...
    if trace.carryInitialized and trace.lastCarry != belief.iCarryFoodOf:
      trace.emit(policy.tick, "carry", %*{"color": teamJson(belief.iCarryFoodOf)})
    trace.carryInitialized = true
    trace.lastCarry = belief.iCarryFoodOf

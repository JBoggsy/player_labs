...
proc selectTarget*(
  ...
  for candidate in candidates:
    ...
    scored.add(scoreTarget(candidate, belief.claimedTarget(candidate.target),
      threat.term, threat.foodDistancePx))
  scored.sort(scoreCmp)
  var best = scored[0]
  if DefensiveTargeting and belief.role == Defender and belief.ownFoodStolen:
  ...
  let carrierImmediate = belief.ownFoodStolen and

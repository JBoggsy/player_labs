...
proc cache*(map: WorldMap, color: Team): Point
...
proc spawnCoverPoint*(map: WorldMap, color: Team, seat, seats: int): Point =
  ...
  ## ordered outward from the food. Fall back to the capture point only when
  ...
  for gy in 0 ..< map.gridH:
    for gx in 0 ..< map.gridW:
      ...
      if zone.contains(point):
        let dx = point.x - food.x
        let dy = point.y - food.y
...
proc cache*(map: WorldMap, color: Team): Point =
  if map.caches.hasKey(color): map.caches[color] else: map.homeCenter(color)

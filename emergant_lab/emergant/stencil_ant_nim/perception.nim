## Sprite-label perception: retained Sprite-v1 scene to a memoryless EmergAntState.
...
proc perceive*(
  ...
  for color in colors:
    let
      atCache = scene.firstWithLabel("food " & color.teamName & " cache")
      carried = scene.firstWithLabel("food " & color.teamName & " carried")
      atCachePos = if atCache.isSome: some(atCache.get.center) else: none(Point)
      carriedPos = if carried.isSome: some(carried.get.center) else: none(Point)
    result.foods[color] = FoodState(
      atCache: atCache.isSome,
      pos: if atCachePos.isSome: atCachePos else: carriedPos,
      carriedPos: carriedPos)
    for mark in scene.objectsWithLabel("pheromone " & color.teamName & " scout"):
      result.pheromones.add(PheromoneMark(
        pos: mark.center, color: color, food: false))
    for mark in scene.objectsWithLabel("pheromone " & color.teamName & " food"):
      result.pheromones.add(PheromoneMark(
        pos: mark.center, color: color, food: true))

  result.iCarryFoodOf = none(Team)
  if self.pos.isSome:
    for color in colors:
      if color == actualTeam:
        continue
      let food = result.foods[color]
      if food.carriedPos.isSome and distance(food.carriedPos.get, self.pos.get) <= CarryDistance:
        result.iCarryFoodOf = some(color)
        break

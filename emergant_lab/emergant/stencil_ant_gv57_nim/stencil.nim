## Stencil-ant GV57: the canonical Emerg-ant reference controller used as the
## compatibility floor for iterative colony improvements.
##
...
when defined(taunt):
  {.error: "taunt support is not bundled in stencil-ant".}
...
proc homeDeepX(team: Team): float =
  ...
  ## vertical center line (195 on the default 1235px arena, scaled with
  ## the map). On a multi-team board: our stated endzone's center x.
  if multiFrameOn():
    return MultiCapture.x
  let deep = float(MapW * 195 div 1235)
...
proc decideEmergAnt(
  ...
  let colonyIndex = bot.slot div max(1, GameTeams)
  let queenDefender =
    when defined(queenRushProbe):
      false
    else:
      not queen and colonyIndex == 7
  let alarmResponder = colonyIndex >= 1 and colonyIndex <= 7
  let queenPos = flagHome(bot.team)
  var queenAlarm = false
  if alarmResponder and not queen and not carrying:
    for mark in client.spriteObjectsWithLabel(
        labelPheromone(myColor, "danger", 3)):
      if dist(client.mapPos(mark), queenPos) <= 220.0:
        queenAlarm = true
        break
  let queenDefenseActive = queenDefender or queenAlarm
  ...
  if carrying:
    let deliveryOffset =
      if (bot.slot div 2) mod 2 == 0: -36.0
      else: 36.0
    target = vec(homeDeepX(bot.team), float(CenterY) + deliveryOffset)
    desiredPheromoneKind = 1
    desiredPheromoneRate = 3
  elif queenDefenseActive:
    # Hold just inside the open-field approach to the queen. The inherited
    # HomeDefender role was previously only a label in this compact GV57
    # controller, leaving every worker away foraging during a queen rush.
    let lane = float(colonyIndex mod 3 - 1) * 34.0
    target = queenPos - vec(homeSign(bot.team) * 68.0, -lane)
    desiredPheromoneKind = 3
    desiredPheromoneRate = 2
  else:
    let foods = client.spriteObjectsWithLabel(LabelFoodPatch)
    if foods.len > 0:
      var best = 1e18
      for food in foods:
        let p = client.mapPos(food)
        let d = dist(p, me)
        if d < best:
          best = d
          target = p
  ...
  when defined(queenRushProbe):
    # Local diagnostic opponent: every worker ignores food and drives straight
    # at the opposing queen. This is never an upload candidate; it provides a
    # repeatable stress test for queen-defense experiments without paid XP.
    if not queen and not carrying:
      target = flagHome(enemy(bot.team))
      desiredPheromoneKind = 2
      desiredPheromoneRate = 3

  # Most workers are dedicated foragers. Symmetric touch fights commonly trade
  # both ants, while one food return scores and wakes a dormant colony copy.
  # The one queen defender is the exception: it intercepts only enemies that
  # enter the nest perimeter. A carrier never abandons its delivery route.
  var bite = false
      ...
      for enemy in client.actorsFor(color):
        ...
        if d < nearestEnemy:
          ...
          if d <= 18.0:
            target = enemy.pos
            bite = true
          if bot.hp < MaxHp or d <= 25.0:
            desiredPheromoneKind = 2
            desiredPheromoneRate = 3
  elif queenDefenseActive and not carrying:
    var nearestThreat = 1e18
    for color in TeamColorNames:
      if color == myColor:
        continue
      for enemy in client.actorsFor(color):
        let queenDistance = dist(enemy.pos, queenPos)
        if queenDistance <= 180.0:
          let defenderDistance = dist(enemy.pos, me)
          if defenderDistance < nearestThreat:
            nearestThreat = defenderDistance
            target = enemy.pos
            bite = defenderDistance <= 18.0
            if bot.hp < MaxHp or queenDistance <= 35.0:
              desiredPheromoneKind = 2
              desiredPheromoneRate = 3
  elif not carrying:
    for color in TeamColorNames:
  ...
  when defined(queenRushProbe):
    if not queen and not carrying:
      var nearestContact = 1e18
      for color in TeamColorNames:
        if color == myColor:
          continue
        for enemyActor in client.actorsFor(color):
          let d = dist(enemyActor.pos, me)
          if d < nearestContact and d <= 28.0:
            nearestContact = d
            target = enemyActor.pos
            bite = d <= 18.0
  ...
  if not carrying and not queenDefenseActive:
  ...
  if bot.antPheromoneRelease:
    ...
    of 0: finalMask = ButtonC or ButtonLeft
    of 1: finalMask = ButtonC or ButtonUp
    of 2: finalMask = ButtonC or ButtonRight
    else: finalMask = ButtonC or ButtonDown
  elif bot.antPheromoneRate != desiredPheromoneRate:
    bot.antPheromoneRate = (bot.antPheromoneRate + 1) mod 4
    bot.antPheromoneRelease = true
    finalMask = ButtonC

  var enemiesVisible = 0
  for color in TeamColorNames:
    if color != myColor:
      enemiesVisible += client.actorsFor(color).len
  let objective =
    when defined(queenRushProbe):
      if queen: "queen_guard"
      elif carrying: "carry_home"
      else: "queen_rush"
    else:
      if queen: "queen_guard"
      elif carrying: "carry_home"
      elif queenAlarm: "queen_defend_alarm"
      elif queenDefender: "queen_defend"
      else: "forage"
  let action =
    if (finalMask and ButtonC) != 0: "pheromone_command"
    elif bite: "bite"
    elif jinked: "stuck_jink"
    else: "move"
  artFrame(FrameSnap(
    ...
    iCarry: carrying, jinked: jinked,
    enemiesVisible: enemiesVisible,
    engageDist: -1,
    mask: finalMask, fired: (finalMask and ButtonA) != 0))
  finalMask
...
proc runBot(url: string) =
  ...
  startProfileTrace()
  echo "stencil-ant slot=", slot, " team=", bot.team, " role=", bot.role, " -> ", endpoint
